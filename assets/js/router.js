...
(function () {
    ...
    function getBasePath() {
        ...
        const knownFiles = ['index.html', 'exam.html', 'editor.html', '404.html'];
        const segments = path.split('/').filter(Boolean);

        if (segments.length > 0 && knownFiles.includes(segments[segments.length - 1])) {
            segments.pop();
        } else if (segments.length > 0 && ['editor', 'exam', 'study'].includes(segments[segments.length - 1])) {
            segments.pop();
        }
    ...
    function buildUrl(page, params = {}) {
        ...
        if (!cleanRoutesSupported()) {
            const fileMap = { home: 'index.html', editor: 'editor.html', exam: 'exam.html', study: 'exam.html' };
    ...
    function getRoute() {
        ...
        return {
            ...
            isClean: ['editor', 'exam', 'study'].includes(firstSegment)
        };
    }
