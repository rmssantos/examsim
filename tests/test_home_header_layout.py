...
class HomeHeaderLayoutTests(unittest.TestCase):
    ...
    def test_home_hero_uses_full_bleed_band_layout(self):
        ...
        self.assertRegex(css, r"\.cr-hero-zone\s*\{[^}]*margin-left:\s*calc\(50% - 50vw\);")
        self.assertRegex(
            css,
            r"\.hero-banner__inner\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1\.45fr\)\s*minmax\(0,\s*1fr\);",
        )
        self.assertIn(".hero-headline", css)
        self.assertIn(".hero-cta-primary", css)
    ...
    def test_local_only_link_can_be_revealed_by_script(self):
        ...
        self.assertRegex(css, r"\.cr-topnav \.theme-controls \.theme-public-link\s*\{[^}]*height:\s*36px;")
        self.assertNotRegex(css, r"\.cr-topnav \.theme-controls \.theme-public-link\s*\{[^}]*display:\s*none;")

    def test_local_only_link_styled_for_the_dark_band(self):
        css = (ROOT / "assets/css/home-v2.css").read_text(encoding="utf-8")

        # The top nav band is graphite in both themes; the link must use the
        # band tokens and expose a hover treatment.
        self.assertRegex(css, r"\.cr-topnav \.theme-controls \.theme-public-link\s*\{[^}]*var\(--rail-line\);")
        self.assertIn(".cr-topnav .theme-controls .theme-public-link:hover", css)
    ...
    def test_library_drag_overlay_follows_theme_tokens(self):
        css = (ROOT / "assets/css/home-v2.css").read_text(encoding="utf-8")
        overlay_start = css.index("body.dragging-file .exam-library-section::after")
        overlay_block = css[overlay_start:css.index("}", overlay_start)]

        # Token-driven colors keep the overlay correct in dark mode without
        # dedicated dark rules.
        self.assertIn("color-mix(in srgb, var(--h-surface)", overlay_block)
        self.assertIn("var(--h-accent-deep)", overlay_block)
