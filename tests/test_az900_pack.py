...
# Types the platform renders; az900 legitimately uses the richer ones.
PLATFORM_TYPES = {"STANDARD", "MULTI", "YES_NO_MATRIX", "SEQUENCE", "DRAG_DROP_SELECT"}


class Az900PackTests(unittest.TestCase):
    def setUp(self):
        self.dump = json.loads((ROOT / "user-content/exams/az900/dump.json").read_text(encoding="utf-8"))
        self.meta = json.loads((ROOT / "user-content/exams/az900/metadata.json").read_text(encoding="utf-8"))

    def test_pack_indexed(self):
        index = json.loads((ROOT / "user-content/exams/index.json").read_text(encoding="utf-8"))
        self.assertIn("az900", index)
    ...
    def test_three_official_domains(self):
        ...
        self.assertEqual(codes, {"AZ900-1", "AZ900-2", "AZ900-3"})
    ...
    def test_free_microsoft_taxonomy(self):
        ...
        self.assertEqual(self.meta.get("certificationCode"), "AZ-900")
