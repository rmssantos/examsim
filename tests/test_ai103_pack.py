...
class Ai103PackTests(unittest.TestCase):
    def setUp(self):
        self.dump = json.loads((ROOT / "user-content/exams/ai103/dump.json").read_text(encoding="utf-8"))
        self.meta = json.loads((ROOT / "user-content/exams/ai103/metadata.json").read_text(encoding="utf-8"))

    def test_pack_indexed(self):
        index = json.loads((ROOT / "user-content/exams/index.json").read_text(encoding="utf-8"))
        self.assertIn("ai103", index)
    ...
    def test_five_official_domains(self):
        ...
        self.assertEqual(codes, {"AI103-1", "AI103-2", "AI103-3", "AI103-4", "AI103-5"})

    def test_pro_preview_funnel(self):
        self.assertEqual(self.meta.get("commercialStatus"), "pro-preview")
        self.assertEqual(self.meta.get("certificationCode"), "AI-103")
        # The homepage gates the "Preview" lock flag (exam-card--preview) on metadata.preview;
        # every pro-preview pack must set it (mirrors az305/az104/saac03).
