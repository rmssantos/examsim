...
class Sc300PackTests(unittest.TestCase):
    def setUp(self):
        self.dump = json.loads((ROOT / "user-content/exams/sc300/dump.json").read_text(encoding="utf-8"))
        self.meta = json.loads((ROOT / "user-content/exams/sc300/metadata.json").read_text(encoding="utf-8"))

    def test_pack_indexed(self):
        index = json.loads((ROOT / "user-content/exams/index.json").read_text(encoding="utf-8"))
        self.assertIn("sc300", index)
    ...
    def test_four_official_domains(self):
        ...
        self.assertEqual(codes, {"SC300-1", "SC300-2", "SC300-3", "SC300-4"})

    def test_pro_preview_funnel(self):
        self.assertEqual(self.meta.get("commercialStatus"), "pro-preview")
        self.assertEqual(self.meta.get("certificationCode"), "SC-300")
