...
# Types the platform renders; ai901 legitimately uses the richer Microsoft formats.
PLATFORM_TYPES = {"STANDARD", "MULTI", "YES_NO_MATRIX", "SEQUENCE", "DRAG_DROP_SELECT"}


class Ai901PackTests(unittest.TestCase):
    def setUp(self):
        self.dump = json.loads((ROOT / "user-content/exams/ai901/dump.json").read_text(encoding="utf-8"))
        self.meta = json.loads((ROOT / "user-content/exams/ai901/metadata.json").read_text(encoding="utf-8"))

    def test_pack_indexed(self):
        index = json.loads((ROOT / "user-content/exams/index.json").read_text(encoding="utf-8"))
        self.assertIn("ai901", index)
    ...
    def test_two_official_domains(self):
    ...
    def test_free_microsoft_taxonomy(self):
        ...
        self.assertEqual(self.meta.get("certificationCode"), "AI-901")
    ...
    def test_recommends_ai103(self):
        rec = self.meta.get("recommendedPro", {})
        self.assertEqual(rec.get("examId"), "ai103")
