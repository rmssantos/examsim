...
# Types the platform renders; sc900 legitimately uses the richer ones.
PLATFORM_TYPES = {"STANDARD", "MULTI", "YES_NO_MATRIX", "SEQUENCE", "DRAG_DROP_SELECT"}


class Sc900PackTests(unittest.TestCase):
    def setUp(self):
        self.dump = json.loads((ROOT / "user-content/exams/sc900/dump.json").read_text(encoding="utf-8"))
        self.meta = json.loads((ROOT / "user-content/exams/sc900/metadata.json").read_text(encoding="utf-8"))

    def test_pack_indexed(self):
        index = json.loads((ROOT / "user-content/exams/index.json").read_text(encoding="utf-8"))
        self.assertIn("sc900", index)

    def test_size_and_official_references(self):
        self.assertGreaterEqual(len(self.dump), 150)
        for q in self.dump:
            ref = q.get("reference", "")
            self.assertTrue(
    ...
    def test_four_official_domains(self):
        ...
        self.assertEqual(codes, {"SCI-1", "SCI-2", "SCI-3", "SCI-4"})
    ...
    def test_free_microsoft_taxonomy(self):
        ...
        self.assertEqual(self.meta.get("certificationCode"), "SC-900")
